...
def scrape_euroleague_data(url):
    # Wykonanie zapytania HTTP
    response = requests.get(url)
    euroleague_data = []
    ...
    if response.status_code == 200:
        ...
        if table:
            ...
            for row in rows:
                ...
                if cells:
                    data = [cell.text.strip() for cell in cells]
                    euroleague_data.append(data)
        else:
            print("Nie znaleziono tabeli na stronie.")
    else:
        print("Błąd podczas pobierania strony:", response.status_code)
    
    return euroleague_data

def scrape_and_save_euroleague_data(url, filename):
    # Pobierz dane z Euroleague
    euroleague_data = scrape_euroleague_data(url)

    # Zapisz dane do pliku JSON
    with open(filename, 'w') as json_file:
        json.dump(euroleague_data, json_file)

    print("Dane z Euroleague zostały zapisane do pliku", filename)

if __name__ == "__main__":
    url_euroleague_standings = "https://www.eurohoops.net/euroleague-standings/"
    filename = "euroleague_standings.json"
    scrape_and_save_euroleague_data(url_euroleague_standings, filename)
